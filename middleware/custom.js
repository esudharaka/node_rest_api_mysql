...
let company = async function (req, res, next) {
    ...
    // if(err) return toErrorResponse(res, "err finding company");
    //
    // if(!company) return toErrorResponse(res, "Company not found with id: "+company_id);
    ...
    //
    // if(!users_array.includes(String(user._id))) return toErrorResponse(res, "User does not have permission to read app with id: "+app_id);
    //
    // req.company = company;
    next();
}
